var R = {}; // the Recurrent library (object-key-value pairs)

(function(global) { // The function is immediately executed, you do not execute it by calling it.
  "use strict";
...
var RL = {};
(function(global) {
  "use strict"; // indicate that the code should be executed in "strict mode". With strict mode, you can not, for example, use undeclared variables.
...
TDAgent.prototype = {
  ...
  updatePolicy: function(s) {
    ...
    var qmax = -1.797693134862316E+308
    var amax = 0;
    for(var i=0,n=poss.length;i<n;i++) {
      var a = poss[i];
      var qval = this.Q[a*this.ns+s];
      if(qval > qmax) { qmax = qval; amax = a;}
    }
    // now update the policy smoothly towards the argmaxy actions
    var psum = 0.0;
    for(var i=0,n=poss.length;i<n;i++) {
      var a = poss[i];
      var target = (a == amax) ? 1.0 : 0.0;
      var ix = a*this.ns+s;
      if(this.smooth_policy_update) {
        // slightly hacky :p
        this.P[ix] += this.beta * (target - this.P[ix]);
        if (this.P[ix] < 0.0) this.P[ix] = 0.0;
        //if (this.P[ix] > 1.0) this.P[ix] = 1.0;
      } else {
        // set hard target
        this.P[ix] = target;
      }
